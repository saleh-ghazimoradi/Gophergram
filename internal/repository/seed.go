...
func generateUsers(num int) []*boiler_models.User {
	users := make([]*boiler_models.User, num)

	for i := 0; i < num; i++ {
		users[i] = &boiler_models.User{
...
func generatePosts(num int, users []*boiler_models.User) []*boiler_models.Post {
	posts := make([]*boiler_models.Post, num)
	for i := 0; i < num; i++ {
		user := users[rand.Intn(len(users))]
		posts[i] = &boiler_models.Post{
			UserID:  user.ID,
...
func generateComments(num int, users []*boiler_models.User, posts []*boiler_models.Post) []*boiler_models.Comment {
	cms := make([]*boiler_models.Comment, num)
	for i := 0; i < num; i++ {
		cms[i] = &boiler_models.Comment{
...
func NewSeederService(userRepository UserRepository, postRepository PostRepository, commentRepository CommentRepository) Seeder {
