...
func testFollowersExists(t *testing.T) {
	...
	e, err := FollowerExists(ctx, tx, o.UserID, o.FollowerID)
	if err != nil {
...
func testFollowersFind(t *testing.T) {
	...
	followerFound, err := FindFollower(ctx, tx, o.UserID, o.FollowerID)
	if err != nil {
		t.Error(err)
	}
...
func testFollowerToOneSetOpUserUsingFollower(t *testing.T) {
	...
	for i, x := range []*User{&b, &c} {
		...
		if exists, err := FollowerExists(ctx, tx, a.UserID, a.FollowerID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	followerDBTypes = map[string]string{`UserID`: `bigint`, `FollowerID`: `bigint`, `CreatedAt`: `timestamp with time zone`}
	_               = bytes.MinRead
)
