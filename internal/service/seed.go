...
type Seeder interface {
	SeedDatabase(ctx context.Context, db *sql.DB) error
}
...
func (s *seederService) SeedDatabase(ctx context.Context, db *sql.DB) error {
...
func generatePosts(num int, users []*service_modles.Users) []*service_modles.Post {
	posts := make([]*service_modles.Post, num)
	for i := 0; i < num; i++ {
		user := users[rand.Intn(len(users))]
		posts[i] = &service_modles.Post{
			UserID:  user.ID,
...
func generateComments(num int, users []*service_modles.Users, posts []*service_modles.Post) []*service_modles.Comments {
	comments := make([]*service_modles.Comments, num)
	for i := 0; i < num; i++ {
		comments[i] = &service_modles.Comments{
			PostID:  posts[rand.Intn(len(posts))].ID,
			UserID:  users[rand.Intn(len(users))].ID,
			Content: commentsText[rand.Intn(len(commentsText))], // Use the correct string for comment content
		}
	}
	return comments
}

func NewSeederService(postService Posts, commentService Comments, userService Users) Seeder {
